...
/**
	...
	Requirements:  Unity 3.5 or later, iOS SDK 5.0 or later.
   ...
   ------------------------------------------------------------------------ */
	...
	Open the generated Unity-iPhone.xcodeproj project file in Xcode.  Edit the AppController.mm file and add the 
	following code at global file scope (e.g. just prior to the line containing "// \-\-\- AppController"):
	...
	<strong>For Unity version 4.1 or later, add this:</strong>
...
public class iVidCapPro : MonoBehaviour {
		...
		CaptureFramerateLock captureFramerateLock)
	{
		...
		if (showDebug) {
			...
			Debug.Log ("BeginRecordingSession: audio settings driverCaps = " + AudioSettings.driverCaps);
		}
	...
	private IEnumerator CaptureRenderTexture() {
		...
		while (sessionInitialized) {
			...
			if (isRecording) {
				...
				//RenderTextureToPNG(rt, "frame1_capture.png");
